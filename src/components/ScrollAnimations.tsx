...
export default function ScrollAnimations() {
  useEffect(() => {
    ...
    const selectors =
      '.problem-card, .service-card, .process-step, .result-card, .team-card, .ideal-box, .pricing-card, .pricing-tier, .credibility-bar, .solution-feature, .image-divider, .audit-split__content, .audit-split__pricing, .solution-visual__image, .solution-visual__content';
    ...
    const handleScroll = () => {
      dividers.forEach((divider) => {
        ...
        if (visible) {
          ...
          if (img) {
            (img as HTMLElement).style.transform = `translateY(${(progress - 0.5) * 40}px) scale(1.05)`;
          }
        }
      });
    };
